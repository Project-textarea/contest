(function (factory) {
    ...
    function DateTimePicker(element, options) {
        var picker = {},
             ...
             * @private
             */
            utilsgetUTCMonthDays = function (date, month) {
            ...
            /**
             * A function to make a string complete with specified characters
             *
             * @param text {String}  old String
             * @param size {Number}  total number of digits required
             * @param ch {String}  characters for padding
            ...
            _compileDateFormat = function (format, date) {
                ...
                switch (ch) {
                    case 'Y':
                        if (len <= 3) {
                            str = (date.getUTCFullYear() + '').slice(2, 4);
                        } else {
                    ...
                    case 'M':
                        if (len > 2) {
                            str = CONSTS.MN[date.getUTCMonth()];
                        } else if (len < 2) {
                            str = date.getUTCMonth() + 1;
                        } else {
                            str = utilsLeftPad(date.getUTCMonth() + 1, 2, '0');
                        }
                        break;
                    case 'D':
                        if (len > 1) {
                            str = utilsLeftPad(date.getUTCDate(), 2, '0');
                        } else {
                            str = date.getUTCDate();
                        }
                        break;
                    ...
                    case 'H': //H(24)
                        if (len > 1) {
                            str = utilsLeftPad(date.getUTCHours(), 2, '0');
                        } else {
                    ...
                    case 'm':
                        if (len > 1) {
                            str = utilsLeftPad(date.getUTCMinutes(), 2, '0');
                        } else {
                    ...
                    case 's':
                        if (len > 1) {
                            str = utilsLeftPad(date.getUTCSeconds(), 2, '0');
                        } else {
            ...
            /**
             * Convert the date object to a string according to the specified format
             * e.g. Thu Dec 12 2013 00:00:00 GMT+0800 + 'yyyy-MM-dd' --> '2013-12-12'
             * @param {Date} date date object
             * @param {String} format date format
             * @return {String} return date string
             */
            ...
            _createButtonPane = function () {
                ...
                console.log('options.todayBtn=>',options.todayBtn)
                if(options.todayBtn==true){
                    // today button
            ...
            _toPrevMonth = function () {
                ...
                } else if (year == sd.getUTCFullYear()) {
                    if (month > sd.getUTCMonth() && month > 0) {
                        _setUTCMonth(month - 1);
                        if (date < sd) {
            ...
            _toNextMonth = function () {
                ...
                } else if (year == edd.getUTCFullYear()) {
                    if (month < edd.getUTCMonth()) {
                        _setUTCMonth(month + 1);
                        if (date > edd) {
            ...
            _toPrevDecade = function () {
                var sd = options.startDate, date = currentDate;
                var year = date.getUTCFullYear() - 10, month = date.getUTCMonth();
                var minMonth, minYear;
                if (sd && year == (minYear = sd.getUTCFullYear())) {
                    minMonth = sd.getUTCMonth();
                }
            ...
            _toNextDecade = function () {
                ...
                if (edd && year == (maxYear = edd.getUTCFullYear())) {
                    maxMonth = edd.getUTCMonth();
                }
            ...
            _setUTCMonth = function (m) {
                var date = currentDate;
                var day = date.getUTCDate(),
                    edd = options.endDate,
                    std = options.startDate;
                var max = utilsgetUTCMonthDays(date, m);
                if (day > max) {
                    date.setUTCDate(max);
                }
                date.setUTCMonth(m);
                if (edd && date > edd) {
                    date.setUTCDate(edd.getUTCDate());
                }
                if (std && date < std) {
                    date.setUTCDate(std.getUTCDate());
                }
            },
            ...
            _loadDateData = function (table, date) {
                ...
                nextDay.setUTCDate(utilsgetUTCMonthDays(nextDay, null) + 1);
                if ((edd && nextDay > edd) || nextDay.getUTCFullYear() > CONSTS.MAXYEAR) {
                ...
                prevDay.setUTCDate(0);
                if ((std && prevDay < std) || prevDay.getUTCFullYear() < CONSTS.MINYEAR) {
                ...
                for (var i = 0; i < 6; i++) {
                    ...
                    var iday;
                    for (var j = 0; j < 7; ++j, date.setUTCDate(iday + 1)) {
                        var $cell = $cells.eq(j);
            ...
            _loadMonthData = function (table, date) {
                ...
                //end date
                if (ed) {
                    if (ed && year == (maxYear = ed.getUTCFullYear())) {
                        maxMonth = ed.getUTCMonth();
                    }
                }
                if (!maxYear || maxYear > CONSTS.MAXYEAR) {
                    maxYear = CONSTS.MAXYEAR;
            ...
            _doTimeDec = function (timetable, input) {
                var inputType = input.data('time');
                if (inputType === 'h') {
                    var hours = (currentDate.getUTCHours() + 23) % 24;
                    currentDate.setUTCHours(hours);
                    timetable.$h.val(utilsLeftPad(hours, 2, '0'));
                } else if (inputType === 'm') {
                    var minutes = (currentDate.getUTCMinutes() + 55) % 60;
                    currentDate.setUTCMinutes(minutes);
                    timetable.$m.val(utilsLeftPad(minutes, 2, '0'));
                } else {
                    var seconds = (currentDate.getUTCSeconds() + 55) % 60;
                    currentDate.setUTCSeconds(seconds);
                    timetable.$s.val(utilsLeftPad(seconds, 2, '0'));
                }
                input.select();
                    ...
                    if (type === 'mouseover') {
                        ...
                        switch (navitype) {
                            ...
                            case NAV['year']:
                                ...
                                var date = currentDate;
                                date.setUTCFullYear($target.text());
                            ...
                            case NAV['month']:
                                ...
                                currentDate.setUTCMonth($target.data('month'));
                                utilsApplyFunc(picker, options.onDateChange, _arguments);
                                break;
                            case NAV['day']:
                                //choose one day
                                ...
                                curDate.setUTCFullYear(cache.showYear);
                                curDate.setUTCMonth(cache.showMonth);
                                curDate.setUTCDate($target.text());
                                utilsApplyFunc(picker, options.onDateChange, _arguments);
                                break;
                            case NAV['plus']:
                                //plus time
        ...
        picker.getValue = function () {
            ...
            if (date && (options.viewMode === viewMode.YMD || options.viewMode === viewMode.YM)) {
                //If time is not included, count time as 00:00:00
